...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!allowedImageTypes.includes(file.type)) {
      return NextResponse.json(
        {error: `Invalid file type. Allowed types are: ${allowedImageTypes.join(", ")}`},{ status: 400 }
      );
    }
    const buffer = Buffer.from(await file.arrayBuffer());
    const groupId = formData.get("friendId") as string;
    ...
    if (!isMember) {
      return NextResponse.json(
        { error: "You can't send this image!" },
        { status: 400 }
      );
    }
    ...
    Promise.all([
      ...
      await postRedis("zadd", `chat:${groupId}`, timestamp, messageId),
    ]);
